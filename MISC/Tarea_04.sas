...
proc sort data=WORK.TRANSFORM(keep=date RetAAPL) out=Work.preProcessedData;
	by date;
run;
...
proc timeseries data=Work.preProcessedData seasonality=7 plots=(series 
	...
	var RetAAPL / accumulate=none transform=none;
run;
...
proc autoreg data=work.transform(keep=date RetAAPL) maxit=200 optimizer=nlp;
	"ARCH(p=1)": model RetAAPL=/ garch=(q=1);
	output out=results_arch1 cev=v;
	"ARCH(p=2)": model RetAAPL=/ garch=(q=2);
	output out=results_arch2 cev=v;
	"GARCH(p=1,q=1)": model RetAAPL=/ garch=(q=1,p=1);
	output out=results_garch_1_1 cev=v;
	"GARCH(p=1,q=2)": model RetAAPL=/ garch=(q=1,p=2);
	output out=results_garch_1_2 cev=v;
	"GARCH(p=2,q=1)": model RetAAPL=/ garch=(q=2,p=1);
	output out=results_garch_2_1 cev=v;
	"GARCH(p=2,q=2)": model RetAAPL=/ garch=(q=2,p=2);
